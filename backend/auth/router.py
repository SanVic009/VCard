...
def signup(request: Request, body: SignupRequest, supabase: Client | None = Depends(get_supabase)):
    logger.info("Signup attempt received")
    return AuthService.signup(email=body.email, password=body.password, supabase=supabase)

@router.post("/login", response_model=AuthResponse)
@limiter.limit("10/minute")
def login(request: Request, body: LoginRequest, supabase: Client | None = Depends(get_supabase)):
    logger.info("Login attempt received")
    return AuthService.login(email=body.email, password=body.password, supabase=supabase)

@router.post("/logout", response_model=MessageResponse)
def logout(current_user: dict = Depends(get_current_user), supabase: Client | None = Depends(get_supabase)):
    logger.info("Logout requested")
    AuthService.logout(user_id=current_user.get("id"), supabase=supabase)
...
def refresh(request: Request, body: RefreshRequest, supabase: Client | None = Depends(get_supabase)):
    ...
    return AuthService.refresh(refresh_token=body.refresh_token, supabase=supabase)
